...
module ConWay (
    ...
    ClockReducer_1_100 sec_1us(clk,tmpclk1);
    ClockReducer_1_100 sec_100us(tmpclk1,tmpclk2);
    ClockReducer_1_100 sec_10ms(tmpclk2,tmpclk3);
    ClockReducer_1_4 sec_40ms(tmpclk3,tmpclk5);
    ClockReducer_1_16 sec_640ms(tmpclk5,milCLK);
    ...
    CursorHandler cursorHandler(btnL,btnR,btnU,btnD,cursorX,cursorY,curXin,curYin);
    ...
    Graphic graphic(clk,toPrint,cursorX,cursorY,~sw[1],row,col,sw[3]);
...
module NextBoard (
    ...
    NextState N00(init[0], init[12],1'b0,    1'b0,    1'b0,    1'b0,    1'b0,    init[1], init[13],res[0] );
    NextState N01(init[12],init[24],1'b0,    1'b0,    1'b0,    init[0], init[1], init[13],init[25],res[12]);
    NextState N02(init[24],init[36],1'b0,    1'b0,    1'b0,    init[12],init[13],init[25],init[37],res[24]);
    NextState N03(init[36],init[48],1'b0,    1'b0,    1'b0,    init[24],init[25],init[37],init[49],res[36]);
    NextState N04(init[48],init[60],1'b0,    1'b0,    1'b0,    init[36],init[37],init[49],init[61],res[48]);
    NextState N05(init[60],init[72],1'b0,    1'b0,    1'b0,    init[48],init[49],init[61],init[73],res[60]);
    NextState N06(init[72],init[84],1'b0,    1'b0,    1'b0,    init[60],init[61],init[73],init[85],res[72]);
    NextState N07(init[84],1'b0,    1'b0,    1'b0,    1'b0,    init[72],init[73],init[85],1'b0,    res[84]);

    NextState N08(init[1], init[13],init[12],init[0], 1'b0,    1'b0,    1'b0,    init[2], init[14],res[1] );
    NextState N09(init[13],init[25],init[24],init[12],init[0], init[1], init[2], init[14],init[26],res[13]);
